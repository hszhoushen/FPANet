...
def sound_feature_extract(args, sound_path, wave_file):
    sound = sound_inference(args.dataset_name, sound_path)
    ...
    if args.dataset_name == 'ESC50':
        ...
        label = sound_path.split('/')[-2]
        print('label:', label)
        # label = torch.from_numpy(label)
        # label = label.cuda()
    elif args.dataset_name == 'DCASE2021-Audio':
        label = ''
...
def returnCAM(feature_conv, weight_softmax, class_idx):
    ...
    print('feature_conv:', feature_conv.shape, type(feature_conv))          # 1, 1024, 28, 28
    print('weight_softmax:', weight_softmax.shape, type(weight_softmax))    # 13, 1024
    print('class_idx:', class_idx)

    avgpool = torch.nn.functional.adaptive_avg_pool2d(feature_conv, (1, 1))
    print('avgpool:', avgpool.shape, type(weight_softmax))    # 13, 1024
    avgpool = avgpool.reshape(avgpool.shape[0], avgpool.shape[1])
    print('avgpool:', avgpool.shape, type(weight_softmax))    # 13, 1024

    atten_map = torch.mean(feature_conv, dim=1).cpu().numpy()
    print('atten_map:', atten_map.shape, type(atten_map))    # 13, 1024

    size_upsample = (224, 224)
    bz, nc, h, w = feature_conv.shape
    output_cam = []
    cam = atten_map.reshape(h, w)
    # print('cam:', cam.shape)
    ...
    #     idx = 0
    #     # cam = weight_softmax[idx].dot(feature_conv.reshape((nc, h*w)).cpu().numpy())
    #     cam = avgpool[idx].cpu().numpy().dot(feature_conv.reshape((nc, h*w)).cpu().numpy())
    # 
    #     print('cam:',  cam.shape)
    #     cam = cam.reshape(h, w)
    #     print('cam:',  cam.shape)
    ...
    #     cam_img = np.uint8(255 * cam_img)
    #     output_cam.append(cv2.resize(cam_img, size_upsample))
        # print('output_cam.shape:', output_cam.shape)
        
    return output_cam

def draw_atten_visual(args, FPAM_net):
    ...
    # print('FPAM_net:', FPAM_net)
    target_layers = FPAM_net.fpam.fpam.conv2
    print('target_layers:', target_layers)

    # cam = GradCAM(model=FPAM_net, target_layers=target_layers, use_cuda=True)

    
    if args.dataset_name == 'ADVANCE-Visual':

        with torch.no_grad():
            FPAM_net = FPAM_net.cuda()
            FPAM_net.eval()

            data_dir = "/lintianlin_group_v100/lgzhou/dataset/ADVANCE/vision"
            label_lst = ['airport', 'beach', 'bridge', 'farmland', 'forest', 'grassland', 'harbour', 'lake', 'orchard',
                         'residential', 'sparse shrub land', 'sports land', 'train station']
            img_file_lst = ['00063_3.jpg', '00440.jpg', '00224.jpg', '00083_2.jpg', '00343.jpg', '00167_3.jpg',
                             '00731_3.jpg', '00433_2.jpg', '01193_6.jpg', '00133.jpg', '00360_1.jpg', '02179_2.jpg',
                             '00261_1.jpg']

    elif args.dataset_name == 'Places365-7':
        with torch.no_grad():
            FPAM_net = FPAM_net.cuda()
            FPAM_net.eval()

            data_dir = "/lintianlin_group_v100/lgzhou/dataset/Places365-7/val"

            label_lst = ['corridor', 'corridor', 'corridor',
            ...
            img_file_lst = ['corridor/Places365_val_00034934.jpg', 'corridor/Places365_val_00034820.jpg',
                            'corridor/Places365_val_00028101.jpg',
                            'bedroom/Places365_val_00015345.jpg', 'bedroom/Places365_val_00001822.jpg',
                            'bedroom/Places365_val_00009488.jpg',
                            'bedroom/Places365_val_00020357.jpg',
                            'office/Places365_val_00001805.jpg', 'office/Places365_val_00004918.jpg',
                            'office/Places365_val_00008146.jpg',
                            'dining_room/Places365_val_00017334.jpg', 'dining_room/Places365_val_00011164.jpg',
                            'dining_room/Places365_val_00000611.jpg',
                            'living_room/Places365_val_00035644.jpg', 'living_room/Places365_val_00008022.jpg',
                            'living_room/Places365_val_00018413.jpg',
                            'kitchen/Places365_val_00035756.jpg', 'kitchen/Places365_val_00007731.jpg',
                            'kitchen/Places365_val_00004746.jpg',
                            'bathroom/Places365_val_00029507.jpg', 'bathroom/Places365_val_00019364.jpg',
                            'bathroom/Places365_val_00028614.jpg']


    for i in range(len(img_file_lst)):
        if args.dataset_name == 'ADVANCE-Visual':
            image_path = os.path.join(data_dir, label_lst[i], img_file_lst[i])
            fig_dir = os.path.join('visualization', 'atten_visualization', args.dataset_name, label_lst[i])
            if not os.path.isdir(fig_dir):
                os.makedirs(fig_dir)
        else:
            image_path = os.path.join(data_dir, img_file_lst[i])
            fig_dir0 = os.path.join('visualization', 'atten_visualization', args.dataset_name, label_lst[i])
            fig_dir = os.path.join('visualization', 'atten_visualization', args.dataset_name)

            if not os.path.isdir(fig_dir0):
                os.makedirs(fig_dir0)
     
        fig_path = os.path.join(fig_dir, img_file_lst[i])
        ...
        image = torch.from_numpy(image).cuda().reshape(1, channel, height, width)
        print('image:', image.shape, type(image))

        # image = torch.autograd.Variable(image).cuda()

        img_output, atten_output = FPAM_net(image)
        print('img_output:', img_output.shape)
        print('img_output:', img_output)
        print('atten_output:', atten_output.shape)

        h_x = torch.nn.functional.softmax(img_output, dim=1).data.squeeze()
        print('h_x:', h_x.shape)
        probs, idx = h_x.sort(0, True)
        print('probs:', probs, 'idx:', idx)
        prediction = torch.max(img_output, 1)[1]
        ...
        # print('FPAM_net._modules:', FPAM_net._modules)
            
        # FPAM_net.fpam.fpam._modules.get('sigmoid_atten').register_forward_hook(hook_feature)
        # FPAM_net.fpam.fpam.register_forward_hook(hook_feature)
        # print('features_blobs:', features_blobs)
        # print('features_blobs:', features_blobs[0].shape)

        # get the softmax weight
        params = list(FPAM_net.parameters())
        # print('params:', params)
        weight_softmax = np.squeeze(params[-2].data.cpu().numpy())



        CAMs = returnCAM(atten_output, weight_softmax, [idx[0]])
        print('CAMs:', len(CAMs), CAMs[0].shape)
        ...
        result = np.array(heatmap * 0.3) + np.array(ori_img * 0.5)
        # result = np.concatenate(heatmap * 0.3, img * 0.5)
        print('result:', result.shape)

        cv2.imwrite(fig_path, result)
...
def visual_drawcam(image_path, figname, atten_output):
...
def draw_atten_audio(args, FPAM_net):
    os.makedirs('atten_visualization', exist_ok=True)
    os.makedirs(os.path.join('atten_visualization', args.dataset_name), exist_ok=True)
    
    # FPAM_net = load_audio_model(args)
    ...
    if args.dataset_name == 'ESC50':
        ...
        with torch.no_grad():
            ...
            # esc50
            wave_file_lst = ['1-84704-A-39.wav', '1-23996-B-35.wav', '1-26176-A-43.wav', '1-26188-A-30.wav', '1-27403-A-28.wav', '1-36400-A-23.wav',
                             '1-46744-A-36.wav', '1-51433-A-17.wav', '1-51805-G-33.wav', '1-53663-A-24.wav', '1-54747-A-46.wav']

            label_lst = ['glass_breaking', 'washing_machine', 'car_horn', 'door_wood_knock', 'snoring', 'breathing',
            ...
            for i in range(len(wave_file_lst)):
                ...
                sound, label = sound_feature_extract(args, sound_path, wave_file_lst[i])
                ...
                audio_draw_soundsignal(sound_path, Fig_name)
                
                # audio_drawcam(args, sound_path, Fig_name, atten_output)
        ...
        #data_dir = "/lintianlin_group_v100/lgzhou/dataset/ADVANCE/sound"
        data_dir = '/data/lgzhou/dataset/ADVANCE/sound'

        with torch.no_grad():
            FPAM_net = FPAM_net.cuda()
            FPAM_net.eval()

            # label_lst = ['airport', 'beach', 'bridge', 'farmland', 'forest', 'grassland', 'harbour', 'lake', 'orchard', 'residential', 'sparse shrub land', 'sports land', 'train station']
            # wave_file_lst = ['00063_3.wav', '00440.wav', '00224.wav', '00083_2.wav', '00343.wav', '00167_3.wav', '00731_3.wav', '00433_2.wav', '01193_6.wav', '00133.wav', '00360_1.wav', '02179_2.wav', '00261_1.wav']

            label_lst = ['beach', 'bridge', 'farmland', 'forest', 'grassland', 'harbour', 'lake', 'residential']
            wave_file_lst = ['00618.wav', '00527.wav', '00142_4.wav', '00607.wav', '00979_1.wav', '00771_2.wav', '03969_4.wav', '00276.wav']

            for i in range(len(wave_file_lst)):
                sound_path = os.path.join(data_dir, label_lst[i], wave_file_lst[i])
                sound, label = sound_feature_extract(args, sound_path, wave_file_lst[i])
                #_, atten_output = FPAM_net(sound)
                #print('atten_output.shape:', atten_output.shape)  # 1, 1024, 26, 8
        ...
        # data_dir = "/lintianlin_group_v100/lgzhou/dataset/ADVANCE/sound"
        data_dir = '/data/lgzhou/dataset/DCASE2021/audio'
        ...
        with torch.no_grad():
            ...
            label_lst = ['airport', 'bus', 'metro_station', 'metro', 'public_square', 'shopping_mall', 'street_traffic', 'tram']
            wave_file_lst = ['airport-helsinki-3-152.wav', 'bus-barcelona-210-6417.wav', 'metro_station-barcelona-61-1826.wav', 'metro-milan-1143-42612.wav', 
                             'public_square-prague-1075-42315.wav', 'shopping_mall-barcelona-125-3734.wav', 'street_traffic-barcelona-162-4968.wav', 'tram-stockholm-198-5966.wav']
        ...
        # sound, label = sound_feature_extract(args, sound_path, wave_file_lst[0])
...
def audio_draw_soundsignal(soundpath, figname):
    wav, sr = librosa.load(soundpath, sr=16000)
    timemax = librosa.get_duration(y=wav, sr=16000, S=None, n_fft=2048, center=True, filename=None)
    melspec = librosa.feature.melspectrogram(wav, sr, n_fft=1024, hop_length=512, n_mels=64)  # n_mels, frequency domain
    max_freq = librosa.mel_frequencies(n_mels=64)[-1]
...
def draw_atten_image(args, FPAM_net):
    import random
    SEED = 0
...
def advance_loader(args):
    ...
    if args.dataset_name == 'ADVANCE-Audio':
        ...
        # sampler_train = DistributedSampler(dataset) if is_distributed else None
        ...
        sampler_test = torch.utils.data.DistributedSampler(test_dataset, num_replicas=num_tasks, rank=global_rank, shuffle=False)
...
def main(args, config):
    ...
    audio_datasets = ['ESC10', 'ESC50', 'DCASE2019', 'US8K', 'DCASE2021-Audio', 'ADVANCE-Audio']
    audio_visual_datasets = ['DCASE2021-Audio-Visual', 'ADVANCE']
    visual_datasets = ['Places365-7', 'Places365-14', 'MIT67', 'SUN_RGBD', 'SUN397', 'NYUData', 'DCASE2021-Visual']
    ...
    if args.dataset_name != 'DCASE2021-Visual' or args.dataset_name != 'ADVANCE-Visual' or args.dataset_name != 'ADVANCE-Audio':
        traindir = os.path.join(data_dir, 'train')
        valdir = os.path.join(data_dir, 'val')
    ...
    if args.dataset_name == 'ADVANCE-Audio':        
        ...
        sampler_val = torch.utils.data.DistributedSampler(val_dataset, num_replicas=num_tasks, rank=global_rank,
                                                          shuffle=False)
    ...
    # 'DCASE2021-Audio-Visual', 'ADVANCE'
        ...
        if args.dataset_name == 'Places365-7':
            ...
            valdir = '/lintianlin_group_v100/lgzhou/dataset/SUNRGBD'
    ...
    if args.status == 'train':
        if args.dataset_name == 'ADVANCE' or args.dataset_name == 'DCASE2021-Audio-Visual':
            ...
            FPAM_Fusion_net = torch.nn.parallel.DistributedDataParallel(FPAM_Fusion_net, 
                                                                    device_ids=[config.LOCAL_RANK],
                                                                    broadcast_buffers=False, find_unused_parameters=True)

            audio_FPAM_net_without_ddp = audio_FPAM_net.module
            visual_FPAM_net_without_ddp = visual_FPAM_net.module
            FPAM_Fusion_net_without_ddp = FPAM_Fusion_net.module
    ...
    if args.status == 'test':
        ...
        best_model_name = os.path.join('./weights', args.model_name)
        ...
        if args.atten_type == 'fpam' or args.atten_type == 'baseline':
            
            # shape the checkpoint
            # audio_net_state_dict = {str.replace(k, 'module.', ''): v for k, v in checkpoint['audio_FPAM_net_state_dict'].items()}
            # visual_net_state_dict = {str.replace(k, 'module.', ''): v for k, v in checkpoint['visual_FPAM_net_state_dict'].items()}
            # FPAM_Fusion_net_state_dict = {str.replace(k, 'module.', ''): v for k, v in checkpoint['FPAM_Fusion_net_state_dict'].items()}


            # audio_net_state_dict = {k: v for k, v in checkpoint['audio_FPAM_net_state_dict'].items()}
            # visual_net_state_dict = {k: v for k, v in checkpoint['visual_FPAM_net_state_dict'].items()}
            # FPAM_Fusion_net_state_dict = {k: v for k, v in checkpoint['FPAM_Fusion_net_state_dict'].items()}
            
            print('args.dataset_name:', args.dataset_name)

            if args.dataset_name in audio_visual_datasets:
                print('audio-visual model')
                audio_net_state_dict = checkpoint['visual_FPAM_net_state_dict']
                visual_net_state_dict = checkpoint['audio_FPAM_net_state_dict']
                FPAM_Fusion_net_state_dict = checkpoint['FPAM_Fusion_net_state_dict']

                # print('audio_net_state_dict:', type(audio_net_state_dict), audio_net_state_dict.keys())
                # print('visual_net_state_dict:', type(visual_net_state_dict))
                # print('FPAM_Fusion_net_state_dict:', type(FPAM_Fusion_net_state_dict))

                # load the checkpoint to model
                audio_FPAM_net.load_state_dict(visual_net_state_dict)
                visual_FPAM_net.load_state_dict(audio_net_state_dict)
                FPAM_Fusion_net.load_state_dict(FPAM_Fusion_net_state_dict)
                ...
                # obtain train logits (from in dataset)
                if args.dataset_name == 'ADVANCE':
                    train_losses, train_acc1, train_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, train_loader, epoch, data_type='train', OOD_model_type="FPAM", OOD_In_dataset_name="ADVANCE", OOD_out_dataset_name="ADVANCE")
                    logger.info(f"Accuracy of the network on the {len(train_loader)} test images and test loss: {train_acc1:.2f}% and {train_losses:.6f}")
                    test_losses, test_acc1, test_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, test_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="ADVANCE", OOD_out_dataset_name="ADVANCE")
                    logger.info(f"Accuracy of the network on the {len(test_loader)} test images and test loss: {test_acc1:.2f}% and {test_losses:.6f}")
                
                elif args.dataset_name == 'DCASE2021-Audio-Visual':
                    train_losses, train_acc1, train_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, train_loader, epoch, data_type='train', OOD_model_type="FPAM", OOD_In_dataset_name="DCASE2021-Audio-Visual", OOD_out_dataset_name="DCASE2021-Audio-Visual")
                    logger.info(f"Accuracy of the network on the {len(train_loader)} test images and test loss: {train_acc1:.2f}% and {train_losses:.6f}")
                    test_losses, test_acc1, test_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, val_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="DCASE2021-Audio-Visual", OOD_out_dataset_name="DCASE2021-Audio-Visual")
                    logger.info(f"Accuracy of the network on the {len(val_loader)} test images and test loss: {test_acc1:.2f}% and {test_losses:.6f}")

                # obtain test logits (from ood dataset)
                if args.dataset_name == 'ADVANCE':
                    args.dataset_name = 'DCASE2021-Audio-Visual'
                    
                    train_loader, val_loader = dcase2021_loader(args)

                    train_losses, train_acc1, train_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, train_loader, epoch, data_type='train', OOD_model_type="FPAM", OOD_In_dataset_name="ADVANCE", OOD_out_dataset_name="DCASE2021-Audio-Visual")
                    logger.info(f"Accuracy of the network on the {len(train_loader)} test images and test loss: {train_acc1:.2f}% and {train_losses:.6f}")
                    test_losses, test_acc1, test_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, val_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="ADVANCE", OOD_out_dataset_name="DCASE2021-Audio-Visual")
                    logger.info(f"Accuracy of the network on the {len(val_loader)} test images and test loss: {test_acc1:.2f}% and {test_losses:.6f}")
                
                elif args.dataset_name == 'DCASE2021-Audio-Visual':
                    print('ADVANCE args.dataset_name:', args.dataset_name)

                    args.dataset_name = 'ADVANCE'

                    train_loader, val_loader, test_loader = advance_loader(args)

                                
                    test_validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, train_loader, epoch, data_type='train', OOD_model_type="FPAM", OOD_In_dataset_name="DCASE2021-Audio-Visual", OOD_out_dataset_name="ADVANCE")
                    # logger.info(f"Accuracy of the network on the {len(train_loader)} test images and test loss: {train_acc1:.2f}% and {train_losses:.6f}")
                    test_validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, test_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="DCASE2021-Audio-Visual", OOD_out_dataset_name="ADVANCE")
                    # logger.info(f"Accuracy of the network on the {len(test_loader)} test images and test loss: {test_acc1:.2f}% and {test_losses:.6f}")


     

            elif args.dataset_name in ['ADVANCE-Visual', 'ADVANCE-Audio', 'DCASE2021-Audio', 'DCASE2021-Visual']:
                if args.atten_type == 'fpam':
                    print('FPAM model')
                    FPAM_net_state_dict = checkpoint['audio_net_state_dict']
                    FPAM_net.load_state_dict(FPAM_net_state_dict)
                    FPAM_net.eval()
                ...
                # obtain train logits from in dataset
                if args.dataset_name == 'ADVANCE-Visual' or args.dataset_name == 'ADVANCE-Audio':

                    if args.atten_type == 'fpam':
                        if args.dataset_name == 'ADVANCE-Visual':
                            OOD_model_type = 'FPAM_Visual'
                        else:
                            OOD_model_type = 'FPAM_Audio'

                    elif args.atten_type == 'baseline':
                        if args.dataset_name == 'ADVANCE-Visual':
                            OOD_model_type = 'Baseline_Visual'
                        else:
                            OOD_model_type = 'Baseline_Audio'


                    train_losses, train_acc1, train_acc5, conf_mat, f1_score = validate(args, 
                                                                                        FPAM_net,
                                                                                        train_loader,
                                                                                        epoch,
                                                                                        data_type='train',
                                                                                        OOD_model_type=OOD_model_type, 
                                                                                        OOD_In_dataset_name=args.dataset_name, 
                                                                                        OOD_out_dataset_name=args.dataset_name)

                    logger.info(f"Accuracy of the network on the {len(train_loader)} test images and test loss: {train_acc1:.2f}% and {train_losses:.6f}")

                    test_losses, test_acc1, test_acc5, conf_mat, f1_score = validate(args, 
                                                                                    FPAM_net,
                                                                                    test_loader,
                                                                                    epoch,
                                                                                    data_type='val',
                ...
                elif args.dataset_name == 'DCASE2021-Audio' or args.dataset_name == 'DCASE2021-Visual':

                    if args.atten_type == 'fpam':
                        if args.dataset_name == 'DCASE2021-Audio':
                            OOD_model_type = 'FPAM_Audio'
                        else:
                            OOD_model_type = 'FPAM_Visual'
                ...
                # obtain logits from OOD dataset
                if args.dataset_name == 'ADVANCE-Visual':
                    args.dataset_name = 'DCASE2021-Visual'
                    train_loader, val_loader = dcase2021_loader(args)
                    
                elif args.dataset_name == 'ADVANCE-Audio':
                    args.dataset_name = 'DCASE2021-Audio'
                    train_loader, val_loader = dcase2021_loader(args)

                elif args.dataset_name == 'DCASE2021-Visual':
                    args.dataset_name = 'ADVANCE-Visual'
                    train_loader, val_loader, test_loader = advance_loader(args)

                elif args.dataset_name == 'DCASE2021-Audio':
                    args.dataset_name = 'ADVANCE-Audio'
                    train_loader, val_loader, test_loader = advance_loader(args)

                
                if args.dataset_name == 'ADVANCE-Visual' or args.dataset_name == 'ADVANCE-Audio':
                    if args.atten_type == 'baseline' and args.dataset_name == 'ADVANCE-Visual':
                        OOD_model_type = 'Baseline_Visual'
                        OOD_In_dataset_name = 'DCASE2021-Visual'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.atten_type == 'fpam' and args.dataset_name == 'ADVANCE-Visual':
                        OOD_model_type = 'FPAM_Visual'
                        OOD_In_dataset_name = 'DCASE2021-Visual'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.atten_type == 'fpam' and args.dataset_name == 'ADVANCE-Audio':
                        OOD_model_type = 'FPAM_Audio'
                        OOD_In_dataset_name = 'DCASE2021-Audio'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.atten_type == 'baseline' and args.dataset_name == 'ADVANCE-Audio':
                        OOD_model_type = 'Baseline_Audio'
                        OOD_In_dataset_name = 'DCASE2021-Audio'
                        OOD_out_dataset_name = args.dataset_name
                    ...
                    test_validate(args, 
                                  ...
                                  data_type='train',
                                  OOD_model_type=OOD_model_type, 
                                  OOD_In_dataset_name=OOD_In_dataset_name, 
                                  OOD_out_dataset_name=OOD_out_dataset_name)

                    logger.info(f"Accuracy of the network on the {len(train_loader)} test images")
                    ...
                    logger.info(f"Accuracy of the network on the {len(test_loader)} test images")
                
                elif args.dataset_name == 'DCASE2021-Visual' or args.dataset_name == 'DCASE2021-Audio':
                    if args.dataset_name == 'DCASE2021-Visual' and args.atten_type == 'fpam':
                        OOD_model_type = 'FPAM_Visual'
                        OOD_In_dataset_name = 'ADVANCE-Visual'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.dataset_name == 'DCASE2021-Visual' and args.atten_type == 'baseline':
                        OOD_model_type = 'Baseline_Visual'
                        OOD_In_dataset_name = 'ADVANCE-Visual'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.dataset_name == 'DCASE2021-Audio' and args.atten_type == 'fpam':
                        OOD_model_type = 'FPAM_Audio'
                        OOD_In_dataset_name = 'ADVANCE-Audio'
                        OOD_out_dataset_name = args.dataset_name

                    elif args.dataset_name == 'DCASE2021-Audio' and args.atten_type == 'baseline':
                        OOD_model_type = 'Baseline_Audio'
                        OOD_In_dataset_name = 'ADVANCE-Audio'
                        OOD_out_dataset_name = args.dataset_name
                    ...
                    logger.info(f"Accuracy of the network on the {len(train_loader)}")
                    ...
                    logger.info(f"Accuracy of the network on the {len(val_loader)}")
    ...
    if args.status == 'draw_image':
        ...
        FPAM_net = FPAM_net.cuda()
        draw_atten_audio(args, FPAM_net)
        # draw_atten_visual(args, FPAM_net)
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):  # epoch from 1 to epochs
            ...
            # rearrange the sequence of data for training
            if args.dataset_name == 'ADVANCE-Audio' or args.dataset_name == 'DCASE2021-Audio-Visual' or args.dataset_name == 'ADVANCE-Visual' or args.dataset_name == 'DCASE2021-Visual':
                print('advance')
            else:
            ...
            if args.dataset_name in audio_visual_datasets:
                ...
                global_rank = dist.get_rank()
                print('global_rank:', type(global_rank), global_rank)
                val_losses, val_acc1, val_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, val_loader, epoch)
                

            else:
                print('evaluation on validation set')

                val_losses, val_acc1, val_acc5, conf_mat, f1_score = validate(args, FPAM_net, val_loader, epoch)
            ...
            logger.info(f'Max val accuracy: {max_accuracy:.3f}%')
            ...
            if args.dataset_name == 'ADVANCE':
                ...
                global_rank = dist.get_rank()
                print('global_rank:', type(global_rank), global_rank)
                test_losses, test_acc1, test_acc5, conf_mat, f1_score = validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, test_loader, epoch)
                logger.info(
            ...
            if args.dataset_name == 'ESC10' or args.dataset_name == 'ESC50':
            ...
            elif args.dataset_name == 'Places365-7' or args.dataset_name == 'Places365-14':
            ...
            if (epoch % 10 == 0):
                ...
                if args.dataset_name in audio_visual_datasets:
                    ...
                    print('saving the checkpoint:', best_model_path)
                    ...
                    save_checkpoint({
                        ...
                        'audio_FPAM_net_state_dict': audio_FPAM_net_without_ddp.state_dict(),
                        'FPAM_Fusion_net_state_dict': FPAM_Fusion_net_without_ddp.state_dict(),
                        'optimizer': optimizer,
...
def train_one_epoch_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, data_loader, optimizer, criterion, epoch, mixup_fn, lr_scheduler):
    ...
    for idx, (audio, visual, labels, paths) in enumerate(data_loader):
        ...
        if (args.atten_type == 'baseline' or args.atten_type == 'fpam'):
            if args.atten_type == 'fpam':
                ...
                if (len(all_logits)) == 0:
                    all_logits = fpam_output.detach().cpu().numpy()
                    # all_target = target.detach().cpu().numpy()
                else:
                    all_logits = np.concatenate((all_logits, fpam_output.detach().cpu().numpy()), axis=0)
    ...
    np.save('logits_file/target_train.npy', all_true)
    np.save('logits_file/logits_train.npy', all_logits)
...
def test_validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, data_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="", OOD_out_dataset_name=""):
    ...
    with torch.no_grad():
        ...
        for i, (audio, images, labels, paths) in enumerate(data_loader):
            ...
            if (args.atten_type == 'baseline' or args.atten_type == 'fpam'):
                ...
                # print('fpam_output.shape:', fpam_output.shape, fpam_output)
        ...
        # np.save('logits_file/target_' + data_type + '.npy', all_true)
        # np.save('logits_file/logits_' + data_type + '.npy', all_logits)

        np.save(os.path.join('logits_file', OOD_model_type, OOD_In_dataset_name, OOD_out_dataset_name, 'target_' + data_type + '.npy'), all_true)
        np.save(os.path.join('logits_file', OOD_model_type, OOD_In_dataset_name, OOD_out_dataset_name, 'logits_' + data_type + '.npy'), all_logits)

    
def validate_audio_visual(args, FPAM_Fusion_net, audio_FPAM_net, visual_FPAM_net, data_loader, epoch, data_type='val', OOD_model_type="FPAM", OOD_In_dataset_name="", OOD_out_dataset_name=""):
    ...
    with torch.no_grad():
        ...
        # all_logits = torch.tensor(all_logits).cpu().data.numpy()
        ...
        global_rank = dist.get_rank()
        print('global_rank:', type(global_rank), global_rank)
        csv_path = os.path.join('logs', args.dataset_name + '_' + str(global_rank) + '.csv')
        result_lst = []
        with open(csv_path, 'a') as f:
            writer = csv.writer(f)
            print('type(all_true):', type(all_true), len(all_true))
            print('type(all_pred):', type(all_pred), len(all_pred))
            # print('len(result):', len(result))
            # result = all_true + all_pred
            
            for i in range(len(all_true)):
                result_lst.append(all_true[i])
            for i in range(len(all_pred)):
                result_lst.append(all_pred[i])
            
            print('len(result_lst):', len(result_lst))


            writer.writerows([result_lst])

        f1_score = performance_matrix(args, all_true, all_pred)
        # classification_report.update(all_true, all_pred)
...
def test_validate(args, FPAM, data_loader, epoch, data_type='val', OOD_model_type="", OOD_In_dataset_name="", OOD_out_dataset_name=""):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (images, labels, paths) in enumerate(data_loader):
            ...
            if i % args.print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      'Test: [{0}/{1}]\t'
                      'Time {batch_time.val:.3f} ({batch_time.avg:.3f})'.format(epoch, i, len(data_loader), batch_time=batch_time))
...
def validate(args, FPAM, data_loader, epoch, data_type='val', OOD_model_type="", OOD_In_dataset_name="", OOD_out_dataset_name=""):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
class Classification_Report_Meter(object):
    ...
    def performance_metric(self):
        print('self.true:', self.true)
        print('self.pred:', self.pred)
        
        from sklearn import metrics
        precision = metrics.precision_score(self.true, self.pred, average='macro')
        recall = metrics.recall_score(self.true, self.pred, average='macro')
        accuracy = metrics.accuracy_score(self.true, self.pred)
        f1_score = metrics.f1_score(self.true, self.pred, average='macro')
        conf_mat = metrics.confusion_matrix(self.true, self.pred)
        print('conf_mat:', conf_mat)
        per_cls_acc = conf_mat.diagonal() / conf_mat.sum(axis=1)
        print('Precision: {} Recall: {}, Accuracy: {}: ,f1_score: {}'.format(precision * 100, recall * 100,
                                                                             accuracy * 100,
                                                                             f1_score * 100))
