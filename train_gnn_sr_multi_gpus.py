...
def main():
    ...
    FPAM_net = FPAM_net.cuda(args.local_rank)

    # single model
    if (args.fusion == False):
        gcn_max_med_model = GCN_audio_top_med(args).cuda(args.local_rank)
    # fusion model
    else:
        gcn_max_med_model = GCN_max_med_fusion(args).cuda(args.local_rank)


    FPAM_net = torch.nn.parallel.DistributedDataParallel(FPAM_net, device_ids=[args.local_rank], output_device=args.local_rank)
    gcn_max_med_model = torch.nn.parallel.DistributedDataParallel(gcn_max_med_model, device_ids=[args.local_rank], output_device=args.local_rank)
...
def train(args, AFM, gcn_max_med_model, data_loader, optimizer, criterion, epoch):
    ...
    for i, (image, label, path) in enumerate(data_loader):
        ...
        # data to cuda
        image = torch.autograd.Variable(image).cuda(args.local_rank)
        ...
        if(args.fusion == False):
            ...
            optimizer.zero_grad()
            loss.backward(retain_graph=True)    # loss propagation, 保存graph结构，后续还要用
